                ...
                const uint8_t sk[KYBER_INDCPA_SECRETKEYBYTES])
{
  ...
  polyvec b, skpv;
  poly v, mp, mpp;
  uint8_t m0[KYBER_INDCPA_MSGBYTES], hashm[KYBER_SYMBYTES];
  ...
  int16_t x, idx0, idx1;
  uint16_t mask0, mask1, abs0, abs1;
  int fail;

  idx0 = 0, idx1 = 0;
  abs0 = 0xFFFF; abs1 = 0xFFFF;
  for(i = 0; i < KYBER_N; i++)
  {
    x = mp.coeffs[i];
    mask0 = (uint16_t)-(uint16_t)(x < abs0);
    mask1 = (uint16_t)-(uint16_t)((x >= abs0) & (x < abs1));

    idx1 = (int16_t)((mask0 & (uint16_t)idx0) | (mask1 & (uint16_t)i) | (~(mask0 | mask1) & (uint16_t)idx1));
    abs1 = (int16_t)((mask0 & (uint16_t)abs0) | (mask1 & (uint16_t)x) | (~(mask0 | mask1) & (uint16_t)abs1));

    idx0 = (int16_t)((mask0 & (uint16_t)i) | (~mask0 & (uint16_t)idx0));

    abs0 = (int16_t)((mask0 & (uint16_t)x) | (~mask0 & (uint16_t)abs0));
  }
