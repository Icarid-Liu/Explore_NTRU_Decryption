 ...
 * 
 * Modifications:
 * 	  Modified the values of the "SOLVE_END_769_1024" array and the polynomial 
...
const ntru_profile SOLVE_END_257_1024 = {
	257,
	10, 10,
	{ 1, 1, 1, 2, 3, 4,  8,  16, 30, 59, 117 },
	{ 1, 1, 2, 3, 6, 12, 23, 44, 87, 172 },
	{ 1, 1, 1, 2, 3, 3,  3,  3,  4,  6 },
	4,
...
{
	int16_t i, j, t;
	__m256i vdata0, vdata1, vdata2, vdata3, vdatalo, vdatahi, vone32 = _mm256_set1_epi8(1);
	__m128i vdata4, vdata5;
	int8_t __attribute__((aligned(32))) tmp1[DIM_N * 2], tmp2[DIM_N * 2], tmp3[DIM_N];
	int8_t __attribute__((aligned(32))) buf[680];

	shake256_squeezeblocks(buf, 5, state);

	t = 0;
	for(i = 0; i < DIMLEN * 2; i += 32)
	{
		vdata0 = _mm256_load_si256((__m256i *)(buf + t + i));
		for(j = 0; j < 8; j++)
		{
			vdata1 = _mm256_and_si256(vdata0, vone32);
			vdata0 = _mm256_srli_epi16(vdata0, 1);
			_mm256_store_si256((__m256i *)(tmp1 + j * 32 + i * 8), vdata1);
		}
	}
	...
	for(i = 0; i < DIMLEN * 2; i += 32)
	{
		...
		for(j = 0; j < 8; j++)
		{
			vdata1 = _mm256_and_si256(vdata0, vone32);
			vdata0 = _mm256_srli_epi16(vdata0, 1);
			_mm256_store_si256((__m256i *)(tmp2 + j * 32 + i * 8), vdata1);
		}
	}

	t += DIMLEN * 2;
	for(i = 0; i < DIMLEN; i += 32)
	{
		vdata0 = _mm256_load_si256((__m256i *)(buf + t + i));
		for(j = 0; j < 8; j++)
		{
			vdata1 = _mm256_and_si256(vdata0, vone32);
			vdata0 = _mm256_srli_epi16(vdata0, 1);
			_mm256_store_si256((__m256i *)(tmp3 + j * 32 + i * 8), vdata1);
		}
	}

	for(i = 0 ; i < DIM_N; i += 32)
	{
		vdata0 = _mm256_load_si256((__m256i *)(tmp1 + i));
		vdata1 = _mm256_load_si256((__m256i *)(tmp1 + i + DIM_N));
		vdata0 = _mm256_and_si256(vdata0, vdata1);
		vdata1 = _mm256_load_si256((__m256i *)(tmp2 + i));
		vdata2 = _mm256_load_si256((__m256i *)(tmp2 + i + DIM_N));
		vdata1 = _mm256_and_si256(vdata1, vdata2);
		vdata0 = _mm256_sub_epi8(vdata0, vdata1);
		vdata1 = _mm256_load_si256((__m256i *)(tmp3 + i));

		vdata4 = _mm256_extracti128_si256(vdata0, 0);
		vdata2 = _mm256_cvtepi8_epi16(vdata4);
		vdata5 = _mm256_extracti128_si256(vdata1, 0);
		vdata3 = _mm256_cvtepi8_epi16(vdata5);
		vdatalo = _mm256_mullo_epi16(vdata2, vdata3);

		vdata4 = _mm256_extracti128_si256(vdata0, 1);
		vdata2 = _mm256_cvtepi8_epi16(vdata4);
		vdata5 = _mm256_extracti128_si256(vdata1, 1);
		vdata3 = _mm256_cvtepi8_epi16(vdata5);
		vdatahi = _mm256_mullo_epi16(vdata2, vdata3);

		vdata2 = _mm256_packs_epi16(vdatalo, vdatahi);
		_mm256_store_si256((__m256i *)(f + i), vdata2);
	}
}
...
void END_keygen(int8_t *h, int8_t *f, int8_t *g, int8_t *F, int8_t *G, int16_t *w, int8_t *k, void *tmp, size_t tmp_len)
{
	...
	for (;;) 
	{
		...
		mq_poly_mul_ntt_257_mq(tmp2, f_inv, tmp1, muldata_257);

		encode_pk(tmp2, h);

		prof = &SOLVE_END_257_1024;
