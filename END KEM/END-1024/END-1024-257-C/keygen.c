...
void END_keygen(int8_t *h, int8_t *f, int8_t *g, int8_t *F, int8_t *G, int16_t *w, int8_t *k, void *tmp, size_t tmp_len)
{
	...
	int16_t norm;
	int16_t f_inv[DIM_N], tmp1[DIM_N], tmp2[DIM_N];
	...
	int8_t *tF = tG + DIM_N;
	uint32_t *tw = (uint32_t *)(tt32 + DIM_N);
	int sum1 = 0, sum_1 = 0;
	...
	for (;;) 
	{
		do 
		{
            ...
            mq_poly_ntt_257(tmp1);
		} while (mq_poly_inv_ntt_257(f_inv, tmp1));
		...
		do
		{
            ...
            mq_poly_ntt_257(tmp1);
		} while (mq_poly_inv_ntt_257(tmp2, tmp1));

		mq_poly_mul_ntt_257_mq(tmp2, f_inv, tmp1);
