...
void poly_int8_to_int16(int8_t *a, int16_t *b)
{
	...
	for(i=0;i<DIM_N;i+=16)
	{
		...
		y=_mm256_cvtepi8_epi16(x);
		_mm256_storeu_si256((__m256i *)(b+i),y);
	}
}
...
void poly_round(int16_t *a)
{
    int i;
	__m256i tmp;
	__m256i tmp_255 = _mm256_set1_epi16(255);
    for(i = 0; i < DIM_N; i += 16)
    {
		tmp = _mm256_loadu_si256((__m256i *)(a + i));
		tmp = _mm256_mullo_epi16(tmp, tmp_255);
		tmp = _mm256_srli_epi16(tmp, 10);
		_mm256_storeu_si256((__m256i *)(a + i), tmp);
    }
}
...
void poly_deround(int16_t *a)
{
    int i;
	__m256i tmp;
	__m256i tmp_3 = _mm256_set1_epi16(3);
    for(i = 0; i < DIM_N; i += 16)
    {
		tmp = _mm256_loadu_si256((__m256i *)(a + i));
		tmp = _mm256_slli_epi16(tmp, 2);
		tmp = _mm256_add_epi16(tmp, tmp_3);
		_mm256_storeu_si256((__m256i *)(a + i), tmp);
    }
}
...
int16_t poly_norm2(int16_t *a)
{
	...
	for(i = 0; i < DIM_N; i += 16)
    {
		...
		sum += _mm256_extract_epi16(x, 0);
		sum += _mm256_extract_epi16(x, 1);
		sum += _mm256_extract_epi16(x, 2);
		sum += _mm256_extract_epi16(x, 3);
		sum += _mm256_extract_epi16(x, 8);
		sum += _mm256_extract_epi16(x, 9);
		sum += _mm256_extract_epi16(x, 10);
		sum += _mm256_extract_epi16(x, 11);
    }
	return sum;
}
...
// if the distribution is binary return 1
int poly_check_distribution_binary(int16_t * s)
{
	int i;
	__m256i vdata;
	__m256i mask = _mm256_set1_epi16((int16_t)~1);
	__m256i sum = _mm256_setzero_si256();
	for(i = 0; i < DIM_N; i+=16)
	{
		vdata = _mm256_loadu_si256((__m256i *)(s + i));
		vdata = _mm256_and_si256(vdata, mask);
		sum = _mm256_or_si256(sum, vdata);
	}
    return _mm256_testz_si256(sum, sum);
}

void ternary_sample_poly(int8_t *a, keccak_state *state)
{
	int16_t i, j;
	int8_t __attribute__((aligned(32))) buf[384];

	shake128_squeezeblocks(buf, 2, state);
    shake128_squeeze(buf + 336, 48, state);

    __m256i x, y, z, k, mask, ans, and1, and2, con1, con2, set1, setn1;

	and1 = _mm256_set1_epi8(63);
    and2 = _mm256_set1_epi8(3);
	con1 = _mm256_set1_epi8(9);
	con2 = _mm256_set1_epi8(54);
	set1 = _mm256_set1_epi8(1);
	setn1 = _mm256_set1_epi8(-1);

    j = 0;
	for(i = 0; i < DIM_N; i += 128)
	{
		ans = _mm256_setzero_si256();
		x = _mm256_load_si256((__m256i *)(buf + j));
        j += 32;
		k = _mm256_and_si256(x, and1);
		mask = _mm256_cmpgt_epi8(con1, k);
		ans = _mm256_blendv_epi8(ans, setn1, mask);
		mask = _mm256_cmpgt_epi8(k, con2);
		ans = _mm256_blendv_epi8(ans, set1, mask);
		_mm256_store_si256((__m256i *)(a + i), ans);

        ans = _mm256_setzero_si256();
		y = _mm256_load_si256((__m256i *)(buf + j));
        j += 32;
		k = _mm256_and_si256(y, and1);
		mask = _mm256_cmpgt_epi8(con1, k);
		ans = _mm256_blendv_epi8(ans, setn1, mask);
		mask = _mm256_cmpgt_epi8(k, con2);
		ans = _mm256_blendv_epi8(ans, set1, mask);
		_mm256_store_si256((__m256i *)(a + i + 32), ans);

        ans = _mm256_setzero_si256();
		z = _mm256_load_si256((__m256i *)(buf + j));
        j += 32;
		k = _mm256_and_si256(z, and1);
		mask = _mm256_cmpgt_epi8(con1, k);
		ans = _mm256_blendv_epi8(ans, setn1, mask);
		mask = _mm256_cmpgt_epi8(k, con2);
		ans = _mm256_blendv_epi8(ans, set1, mask);
		_mm256_store_si256((__m256i *)(a + i + 64), ans);

        ans = _mm256_setzero_si256();
        x = _mm256_and_si256(_mm256_srli_epi16(x, 6), and2);
        y = _mm256_and_si256(_mm256_srli_epi16(y, 6), and2);
        z = _mm256_and_si256(_mm256_srli_epi16(z, 6), and2);
        z = _mm256_xor_si256(_mm256_slli_epi16(x, 4), z);
        z = _mm256_xor_si256(_mm256_slli_epi16(y, 2), z);
		mask = _mm256_cmpgt_epi8(con1, z);
		ans = _mm256_blendv_epi8(ans, setn1, mask);
		mask = _mm256_cmpgt_epi8(z, con2);
		ans = _mm256_blendv_epi8(ans, set1, mask);
		_mm256_store_si256((__m256i *)(a + i + 96), ans);
	}
}
