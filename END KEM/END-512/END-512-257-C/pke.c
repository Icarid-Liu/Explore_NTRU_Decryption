...
{
    int16_t i, tmp, u0, u1, idx0, idx1, idx2, idx3;
    ...
    for(i = 0; i < DIM_N; i++)
    {
        tmp6[i] = montgomery_reduce_257(2 * c[i] - tmp1[i]); //tmp6 = 2c-1_257
    }
    mq_poly_mul_ntt_257(c1, tmp6, tmp2); //c1 = (2c-1)f_257
    mq_poly_mul_ntt_257(tmp2, tmp1, tmp3); //tmp2 = g.1_257

    mask1 = 0;
    for(i = 0; i < DIM_N; i++)
    {
        c1[i] = montgomery_reduce_257(c1[i] - tmp2[i]);
        mask1 ^= ((f[i] + g[i]) & 1);
    }
    mq_poly_intt_257(c1); //c1 = cf-g.1_mod257
    ...
    mq_poly_mul_ntt_257(c2, tmp6, tmp4); //c2 = (2c-1)F_257
    mq_poly_mul_ntt_257(tmp4, tmp1, tmp5); //tmp4 = G.1_257

    mask1 = 0;
    for(i = 0; i < DIM_N; i++)
    {
        c2[i] = montgomery_reduce_257(c2[i] - tmp4[i]);
        mask1 ^= ((F[i] + G[i]) & 1);
    }
    mq_poly_intt_257(c2); //c2 = (2c-1)F-G.1_mod257
    ...
    for(i = 0; i < DIM_N; i++)
    {
        tmp2[i] = fqmul_257(w[i], 1); //tmp2 = w
        tmp3[i] = fqmul_12289(w[i], 4091); //tmp3 = w
    }
    ...
    for(i = 0; i < DIM_N; i++)
    {
        tmp3[i] = fqmul_769(w[i], 171); //tmp3 = w;
    }
    ...
    memcpy(tmp4, c1, DIM_N * sizeof(int16_t)); // tmp4 = c1
    mq_poly_ntt_769(tmp4); //tmp4 = c1_769
    mq_poly_mul_ntt_769(tmp5, tmp4, tmp3); //tmp5 = c1(QF-wf)_769
